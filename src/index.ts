...
async function main() {
	...
	const toPrivateTx = await privateWallet.toPrivateBuild(dolphinId, thousandDol, alice, alice.address);
	...
	const toPublicTx = await privateWallet.toPublicBuild(dolphinId, fiveHundredDol, alice, alice.address);
	...
	const afterToPublicPrivateBalance = await privateWallet.getPrivateBalance(dolphinId);
	console.log("private balance of DOL: ", afterToPublicPrivateBalance?.toString());
